...
actor Main
  ...
  new create(env: Env) => 
    ...
    Assert(check(err, arch, env.out), "ERR: Support gzip compression")?
    err = @archive_read_support_compression_bzip2(arch)
    Assert(check(err, arch, env.out), "ERR: Support bzip compression")?
    err = @archive_read_support_compression_lzip(arch)
    Assert(check(err, arch, env.out), "ERR: Support lzip compression")?
    err = @archive_read_support_format_all(arch)
    Assert(check(err, arch, env.out), "ERR: Support zip format")?
    err = @archive_read_open_filename(arch, filename.cstring(), 10240)
    Assert(check(err, arch, env.out), "ERR: open File")?
